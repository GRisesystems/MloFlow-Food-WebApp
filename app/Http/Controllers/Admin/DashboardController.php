...
class DashboardController extends MainAdminController
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->usertype == 'Owner') {
            ...
            $categories_count = Categories::where(['restaurant_id' => $restaurant_id])->count() ?? 0;

            $menu_count = Menu::where(['restaurant_id' => $restaurant_id])->count() ?? 0;

            $order_count = Order::where(['restaurant_id' => $restaurant_id])->count() ?? 0;

            $review_count = Review::where(['restaurant_id' => $restaurant_id])->count() ?? 0;
